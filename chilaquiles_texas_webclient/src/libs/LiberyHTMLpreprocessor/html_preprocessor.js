...
/**
 * @typedef {Object} HTMLEventListenerItemParams
 * @property {string} event_name
 * @property {Function} event_listener
 */
...
class HTMLEventListenerItem {
    ...
    /**
     * @param {HTMLEventListenerItemParams} param0
     */
...
/**
 * @typedef {Object} HTMLPreprocessRuleParams
 * @property {Object<string, string>[]} attributes - The attributes to add to the tag
 * @property {string[]} class_names - The class names to add to the tag
 * @property {HTMLEventListenerItem[]} listeners - The listeners to add to the tag
 */
...
class HTMLPreprocessRule {
    ...
    /**
     * @param {HTMLPreprocessRuleParams} param0 
     */
    ...
    ProcessNode = tag => {
        ...
        this.#listeners.forEach(listener => {
            // @ts-ignore - i really hate ts complaining about event listeners..
...
export class LiberyHTMLPreprocessor {
    ...
    #applyExactRule = node => {
        ...
        if (rule !== undefined && node instanceof Element) {
    ...
    #processHeadlines = node => {
        ...
        if (headline_rule !== undefined && node instanceof Element) {
            node = headline_rule.ProcessNode(node);
        }

        if (special_headline_rule !== undefined && node instanceof Element) {
    ...
    setRules = rules => {
        ...
        rules.forEach(rule => {
            ...
            if (rule.event_handlers !== undefined) {
                rule.event_handlers.forEach(handler => {

                    for(let event_handler_entry of Object.entries(handler)) {
                        rule_listeners.push(new HTMLEventListenerItem({
                            event_name: event_handler_entry[0],
                            event_listener: event_handler_entry[1]
                        }));
                    }
                });
            }
